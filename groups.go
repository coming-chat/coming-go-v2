...
func sendGroupV2Helper(hexid string, msg string, attachmentPointer *attachments.AttachmentPointerV3, timer uint32) (uint64, error) {
	var ts uint64
	var err error
...
func sendGroupHelper(hexid string, msg string, attachmentPointer *attachments.AttachmentPointerV3, timer uint32) (uint64, error) {
	var ts uint64
	var err error
	g, ok := groups[hexid]
	if !ok {
...
func SendGroupAttachment(hexid string, msg string, r io.Reader, timer uint32) (uint64, error) {
	...
	attachmentPointer, err := attachments.UploadAttachment(r, ct)
	if err != nil {
		return 0, err
	}
...
func SendGroupVoiceNote(hexid string, msg string, r io.Reader, timer uint32) (uint64, error) {
	ct, r := MIMETypeFromReader(r)
	attachmentPointer, err := attachments.UploadVoiceNote(r, ct)
	if err != nil {
		return 0, err
	}
	return sendGroupHelper(hexid, msg, attachmentPointer, timer)
